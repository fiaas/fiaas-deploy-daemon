...
class Secrets(object):
    ...
    def _strongbox_secrets_spec(self, app_spec):
        strongbox_params = {
    ...
    def apply(self, deployment, app_spec):
        ...
        if not secret_specs:
            if self._generic_init.supports("strongbox") and app_spec.strongbox.enabled:
                secret_specs = [self._strongbox_secrets_spec(app_spec)]
            elif self._generic_init.supports("default"):
                secret_specs = [SecretsSpec(type="default", parameters={}, annotations={})]
