...
class RoleBindingDeployer:
    ...
    def deploy(self, app_spec: AppSpec, labels):
        ...
        counter = 1
        self._create_role_bindings(app_spec, self._list_of_roles, "Role", counter, custom_annotations, custom_labels, role_bindings)
        self._create_role_bindings(app_spec, self._list_of_cluster_roles, "ClusterRole", counter, custom_annotations, custom_labels,
                                   role_bindings)
            ...
            if role_binding:
                if self._owned_by_fiaas(role_binding):
                    role_binding.metadata = ObjectMeta(
                        name=role_binding.metadata.name, namespace=namespace, labels=custom_labels, annotations=custom_annotations
                    )
    ...
    def _clean_not_needed_role_bindings(self, role_bindings: list[RoleBinding]):
        for role_binding in role_bindings:
            if not self._check_if_exists(role_binding):
                if self._owned_by_fiaas(role_binding):
    ...
    def _check_if_exists(self, role_binding):
