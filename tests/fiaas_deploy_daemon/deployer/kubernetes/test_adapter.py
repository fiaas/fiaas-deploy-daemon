...
class TestK8s(object):
        ...
        [
            ...
            (
                [
                    ...
                    {"hard": {"pods": "0"}, "scopes": [NotBestEffort]},
                ],
                True,
                True,
            ),
    ...
    def test_pass_to_deployment(
        ...
        resource_quotas = [
            ResourceQuota(
                metadata=ObjectMeta(name="quota-{}".format(i), namespace=app_spec.namespace),
    ...
    def test_pass_to_role_binding(
        ...
        if enable_service_account_per_app:
            pytest.helpers.assert_any_call(role_binding_deployer.deploy, app_spec, labels)
        else:
