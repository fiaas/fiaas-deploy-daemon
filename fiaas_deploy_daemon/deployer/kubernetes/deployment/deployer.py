...
class DeploymentDeployer(object):
    ...
    def __init__(
        ...
        self._search_domains = config.search_domains
    ...
    def deploy(self, app_spec, selector, labels, besteffort_qos_is_required):
        ...
        dns_config = None
        if self._search_domains:
            dns_config = PodDNSConfig(
                searches=self._search_domains,
            )
        ...
        pod_metadata = ObjectMeta(
            name=app_spec.name, namespace=app_spec.namespace, labels=pod_labels, annotations=app_spec.annotations.pod
        )
            ...
            try:
                ...
                if deployment.spec.replicas > 0:
                    ...
                    LOG.info(
                        ...
                        " is different (%d) for %s",
                        app_spec.autoscaler.min_replicas,
        ...
        deployment_strategy = DeploymentStrategy(
            rollingUpdate=RollingUpdateDeployment(maxUnavailable=self._max_unavailable, maxSurge=self._max_surge)
        )
        ...
        self._datadog.apply(
            deployment, app_spec, besteffort_qos_is_required, self._pre_stop_delay + self.DATADOG_PRE_STOP_DELAY
        )
                ...
                EnvVar(
                    ...
                    valueFrom=EnvVarSource(
                        resourceFieldRef=ResourceFieldSelector(
                            containerName=app_spec.name, resource="requests.cpu", divisor=1
                        )
                    ),
                ),
                EnvVar(
                    ...
                    valueFrom=EnvVarSource(
                        resourceFieldRef=ResourceFieldSelector(
                            containerName=app_spec.name, resource="requests.memory", divisor=1
                        )
                    ),
                ),
                EnvVar(
                    ...
                    valueFrom=EnvVarSource(
                        resourceFieldRef=ResourceFieldSelector(
                            containerName=app_spec.name, resource="limits.cpu", divisor=1
                        )
                    ),
                ),
                EnvVar(
                    ...
                    valueFrom=EnvVarSource(
                        resourceFieldRef=ResourceFieldSelector(
                            containerName=app_spec.name, resource="limits.memory", divisor=1
                        )
                    ),
                ),
                EnvVar(
...
def _add_status_label(labels):
    copy = labels.copy()
    copy.update({"fiaas/status": "active"})
    return copy
...
def _make_probe(check_spec):
    probe = Probe(
        initialDelaySeconds=check_spec.initial_delay_seconds,
    ...
    if check_spec.http:
        probe.httpGet = HTTPGetAction(
            ...
            httpHeaders=[
                HTTPHeader(name=name, value=value) for name, value in list(check_spec.http.http_headers.items())
            ],
        )
    elif check_spec.tcp:
        ...
        env.update(
            {
                ...
                "CONSTRETTO_TAGS": ",".join(
                    ("kubernetes-{}".format(config.environment), "kubernetes", config.environment)
                ),
            }
        )
    return env
