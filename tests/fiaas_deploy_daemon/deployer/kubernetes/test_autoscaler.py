...
class TestAutoscalerDeployer(object):
    ...
    def test_new_autoscaler_with_custom_labels_and_annotations(self, deployer, post, app_spec):
        ...
        app_spec = app_spec._replace(replicas=4)
        app_spec = app_spec._replace(resources=ResourcesSpec(limits=[], requests=ResourceRequirementSpec(cpu=1, memory=1)))
        app_spec = app_spec._replace(labels={"horizontal_pod_autoscaler": {"custom": "label"}}, annotations={"horizontal_pod_autoscaler": {"custom": "annotation"}})

        deployer.deploy(app_spec, LABELS)

        expected_autoscaler = {
            'metadata': pytest.helpers.create_metadata('testapp', labels={"autoscaler_deployer": "pass through", "custom": "label"}, annotations={"custom": "annotation"}),
            'spec': {
                "scaleTargetRef": {
                    "kind": "Deployment",
                    "name": "testapp",
