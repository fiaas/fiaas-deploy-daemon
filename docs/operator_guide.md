...
* `tls-certificate-issuer-disable-for-domain-suffixes` disable tls for specified domain suffixes when `use-ingress-tls` is set to `default_on`. for example if you want to enable tls for all domain suffixes except `internal.example.com` set `tls-certificate-issuer-disable-for-domain-suffixes internal.example.com` .
